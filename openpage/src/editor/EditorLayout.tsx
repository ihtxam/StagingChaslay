...
function useEmbedHostBridge() {
  ...
  useEffect(() => {
    ...
    if (!readySent.current) {
      ...
      postToParent({ source: 'openpage', type: 'openpage:ready' })
    }

    function onMessage(ev: MessageEvent) {
      const data = ev.data as EmbedParentMessage
      if (!data || data.source !== EMBED_SOURCE_PARENT) return

      if (data.type === 'openpage:init') {
        let next: SiteConfig
        if (data.config && Array.isArray(data.config.blocks)) {
          next = data.config
      ...
      if (data.type === 'openpage:requestSave') {
  ...
  }, [activeProjectId, addProject, config.name, setActiveProject, setConfig])
}
