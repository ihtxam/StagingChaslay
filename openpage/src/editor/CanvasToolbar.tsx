...
export function CanvasToolbar() {
  ...
  async function handleExport() {
    ...
    try {
      if (embed) {
        const html = exportConfigHtml(config)
        postToParent({ source: 'openpage', type: 'openpage:saved', config, html })
        toast('Saved to FoodTruckPOS')
        return
      }
      ...
      toast.error(embed ? 'Save failed' : 'Export failed')
    } finally {
      setExporting(false)
    }
  }
    ...
    <div className="h-10 bg-bg-1 border-b border-border-default flex items-center px-3 gap-1">
      ...
      <div className="flex items-center gap-1.5 text-xs text-text-3 shrink-0">
        {!embed ? (
          ...
          <span className="text-text-2">OpenPage</span>
        )}
        <span>/</span>
        ...
        >
          {exporting ? (
            <>
              ...
              <span>{embed ? 'Saving...' : 'Exporting...'}</span>
            </>
          ) : (
            <>
              <Download size={12} />
              <span>{embed ? 'Save' : 'Export'}</span>
            </>
          )}
        </button>
      </div>
