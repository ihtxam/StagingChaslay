...
function Send-RawToPrinter {
    ...
    try {
        ...
        try {
            ...
            # Bluetooth/COM serial drivers often overflow on a single large WritePrinter call.
            # Chunk like the Android POS client (512 bytes + brief pause) so multi-item kitchen
            # tickets and the trailing GS V cut command are not dropped.
            $chunkSize = 512
            $offset = 0
