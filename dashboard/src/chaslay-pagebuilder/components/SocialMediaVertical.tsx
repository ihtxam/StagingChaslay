...
export const SocialMediaVertical: React.FC<SocialMediaProps> & {
  craft: {
  ...
  const getIconContainerStyle = (): React.CSSProperties => {
    ...
    if (mergedProps.iconStyle === 'circle') {
      ...
      base.backgroundColor = mergedProps.textColor;
  ...
  return (
    ...
    >
      {mergedProps.title && (
        <h3 style={{ color: mergedProps.textColor, fontSize: '24px', fontWeight: 600, marginBottom: '32px' }}>
          {mergedProps.title}
        </h3>
      )}
      <div style={{ display: 'flex', flexDirection: 'column', gap: `${mergedProps.gap}px`, alignItems: mergedProps.alignment === 'center' ? 'center' : mergedProps.alignment === 'right' ? 'flex-end' : 'flex-start' }}>
        {activeLinks.map(({ key, label, Icon }) => (
          ...
          >
            ...
            {mergedProps.showLabels && (
              <span style={{ color: mergedProps.textColor, fontSize: '16px', fontWeight: 500 }}>{label}</span>
            )}
          </a>
        ))}
      </div>
...
const SocialMediaVerticalSettings: React.FC = () => {
...
SocialMediaVertical.craft = {
  props: defaultProps,
  related: { settings: SocialMediaVerticalSettings },
  displayName: 'Social Media Vertical',
};
