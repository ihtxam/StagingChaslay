...
export default function WebPos({ appMode = true }: { appMode?: boolean }) {
  ...
  useEffect(() => {
    initWebPosLogging({
      ...
      onAutoSend: (ok) => {
        if (ok) toast.success(t('webPosLogsSent'));
      },
    });
  }, [locale, webposStaff?.name, webposStaff?.roleName, merchant?.name, merchant?.businessName, t]);

  const offlineSyncSigRef = useRef('');
  useEffect(() => {
    const sig = `${offlineSync.online}|${offlineSync.pendingCount}|${offlineSync.failedCount}|${offlineSync.lastError || ''}`;
    if (offlineSyncSigRef.current && offlineSyncSigRef.current !== sig) {
      logWebPosEvent(
        'offline',
        `Sync state online=${offlineSync.online} pending=${offlineSync.pendingCount} failed=${offlineSync.failedCount}${offlineSync.lastError ? ` err=${offlineSync.lastError}` : ''}`,
        offlineSync.failedCount > 0 || offlineSync.lastError ? 'warn' : 'info'
      );
    }
    offlineSyncSigRef.current = sig;
  }, [offlineSync]);

  const agentOkLoggedRef = useRef<boolean | null>(null);
  useEffect(() => {
    if (agentOkLoggedRef.current === agentOk) return;
    agentOkLoggedRef.current = agentOk;
    logWebPosEvent('print', agentOk ? 'Print agent online' : 'Print agent offline', agentOk ? 'info' : 'warn');
  }, [agentOk]);
  ...
  const onStaffPinSuccess = async (staff: {
    ...
    notifyWebPosStaffSessionChanged();
    logWebPosEvent('staff', `Signed in as ${staff.name} (${staff.roleName || 'staff'})`);
