...
export default function OnlineShop() {
  ...
  const [ruleMinOrder, setRuleMinOrder] = useState('35');
  const [ruleDeliveryFee, setRuleDeliveryFee] = useState('0');
  const [ruleFreeDelivery, setRuleFreeDelivery] = useState('35');
  ...
  const onSaveZipRule = async (e: FormEvent) => {
    ...
    try {
      const payload = {
        city: ruleCity.trim(),
        zipCode: ruleZipCode.trim(),
        minOrderAmount: Number(ruleMinOrder),
        deliveryFee: Number(ruleDeliveryFee),
        freeDeliveryMinOrder: Number(ruleFreeDelivery),
        estimatedMinutes: Number(ruleEta),
      };
      if (editingZipRuleId) {
        await api.put(`/delivery-zip-rules/${editingZipRuleId}`, payload);
        toast.success('Postal code rule updated');
      } else {
        await api.post('/delivery-zip-rules', payload);
        toast.success('Postal code rule created');
      }
      clearZipRuleEditor();
      await load();
    } catch (error: any) {
      toast.error(error.response?.data?.error || 'Failed to save postal code rule');
    } finally {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="card space-y-4">
        ...
        <form onSubmit={onSaveZipRule} className="space-y-3">
          <div className="text-sm font-medium">
            {editingZipRuleId ? 'Editing postal code rule' : 'Add postal code rule'}
          </div>
          <div className="grid grid-cols-2 md:grid-cols-5 gap-3">
            ...
            <input
              ...
              type="number"
              step="0.01"
              placeholder="Free delivery CHF"
              value={ruleFreeDelivery}
              onChange={(e) => setRuleFreeDelivery(e.target.value)}
              required
            />
          </div>
          <div className="flex flex-wrap gap-2">
            <button type="submit" className="btn-primary" disabled={savingZipRule}>
              {savingZipRule ? 'Saving…' : editingZipRuleId ? 'Update rule' : 'Add rule'}
            </button>
            {editingZipRuleId && (
              <button type="button" className="btn-secondary" onClick={clearZipRuleEditor}>
                Cancel
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="text-left border-b">
              ...
              <th className="py-2">Delivery fee</th>
            ...
            {zipRules.length === 0 && (
              <tr>
                <td colSpan={6} className="py-6 text-gray-500">
                  No postal code rules yet — add one above.
                </td>
              </tr>
            )}
            {zipRules.map((rule) => (
              ...
              >
                <td className="py-3 font-medium">{rule.city || rule.name}</td>
                <td className="py-3">{rule.zipCode || '—'}</td>
                <td className="py-3">CHF {Number(rule.deliveryFee).toFixed(2)}</td>
                <td className="py-3">CHF {Number(rule.minOrderAmount).toFixed(2)}</td>
                <td className="py-3">
                  CHF {Number(rule.freeDeliveryMinOrder ?? rule.minOrderAmount).toFixed(2)}
                </td>
                <td className="py-3 text-right space-x-3">
                  <button
                    type="button"
