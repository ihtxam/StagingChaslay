...
export default function WebPosRetailCategorySidebar({
          ...
          {items.map((item) => {
            ...
            return (
              <button
                ...
                className={`shrink-0 rounded-xl px-4 py-3 text-sm font-bold uppercase tracking-wide ${
                  active
  ...
  return (
    <aside
      className={`webpos-retail-sidebar hidden shrink-0 flex-col border-r border-[var(--webpos-border)] bg-[var(--webpos-bg)] lg:flex lg:order-1 ${
            ...
            return (
              <li key={String(item.id)}>
                <button
                  ...
                  onClick={() => onCategoryChange(item.id)}
                  title={item.label}
                  className={`flex w-full touch-manipulation items-center gap-2 rounded-xl px-3 py-4 text-left text-base font-semibold transition ${
                    active
                      ? 'bg-[var(--webpos-accent)] text-white shadow-sm'
                      : 'text-stone-700 hover:bg-stone-100'
                  } ${collapsed ? 'justify-center px-1 py-3' : ''}`}
                >
                  {item.icon === 'gift' ? (
