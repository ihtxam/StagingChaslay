...
export async function generateDeliverySlipEscPos(opts: DeliverySlipOpts): Promise<Uint8Array> {
  ...
  if (opts.driverClaimUrl?.trim()) {
    lines.push(centerLine(L.scanDriver, width));
  }
  lines.push(centerLine(L.nonFiscalTicket, width), '', '');
  ...
  const driverQr = opts.driverClaimUrl?.trim()
    ? (await buildLabeledReceiptQrRasterEscPos({
        label: L.scanDriver,
