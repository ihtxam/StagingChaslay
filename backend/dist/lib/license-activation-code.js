"use strict";
Object.defineProperty(exports, "__esModule", { value: true });
exports.compactActivationCode = compactActivationCode;
exports.formatShortActivationCode = formatShortActivationCode;
exports.normalizeActivationCode = normalizeActivationCode;
exports.activationCodeLookupKeys = activationCodeLookupKeys;
exports.isPlaceholderDeviceId = isPlaceholderDeviceId;
exports.isUnusedIssuedDevice = isUnusedIssuedDevice;
exports.canRebindLicenseDevice = canRebindLicenseDevice;
/** Canonical alphanumeric form of a POS activation / license code. */
function compactActivationCode(code) {
    return String(code ?? "")
        .trim()
        .toUpperCase()
        .replace(/[^A-Z0-9]/g, "");
}
/** Reborn short codes are 12 hex chars grouped as XXXX-XXXX-XXXX. */
function formatShortActivationCode(compact) {
    const c = compactActivationCode(compact);
    if (c.length === 12) {
        return `${c.slice(0, 4)}-${c.slice(4, 8)}-${c.slice(8, 12)}`;
    }
    return c.match(/.{1,4}/g)?.join("-") ?? c;
}
/**
 * Normalize what the user typed so it matches stored Reborn codes.
 * Strips spaces, accepts upper/lower case, and inserts hyphens for 12-char codes.
 */
function normalizeActivationCode(code) {
    const compact = compactActivationCode(code);
    if (!compact)
        return "";
    if (compact.length === 12)
        return formatShortActivationCode(compact);
    return String(code ?? "").trim().toUpperCase().replace(/\s+/g, "");
}
/** Candidate license_key values to try before a compact SQL fallback. */
function activationCodeLookupKeys(code) {
    const compact = compactActivationCode(code);
    const raw = String(code ?? "").trim().toUpperCase().replace(/\s+/g, "");
    const keys = new Set();
    if (raw)
        keys.add(raw);
    if (compact)
        keys.add(compact);
    if (compact.length === 12)
        keys.add(formatShortActivationCode(compact));
    return [...keys];
}
function isPlaceholderDeviceId(externalId) {
    return /^POS-/i.test(String(externalId || "").trim());
}
/** Issued in admin but never actually activated from a tablet. */
function isUnusedIssuedDevice(device) {
    if (!device)
        return true;
    if (device.lastSync)
        return false;
    return !String(device.appVersion || "").trim();
}
function canRebindLicenseDevice(device, incomingMatchesStored) {
    if (!device)
        return true;
    if (incomingMatchesStored)
        return true;
    if (isPlaceholderDeviceId(device.deviceId || ""))
        return true;
    return isUnusedIssuedDevice(device);
}
//# sourceMappingURL=license-activation-code.js.map