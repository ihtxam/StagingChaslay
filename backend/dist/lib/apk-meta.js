    ...
    while (offset < buf.length - 30) {
        ...
        if (entry === entryName) {
            ...
            if (compMethod === 0)
                return data;
            if (compMethod === 8)
                return zlib_1.default.inflateRawSync(data);
            return null;
        }
...
function scanUtf16Semver(manifest) {
    const found = new Set();
    for (let i = 0; i < manifest.length - 6; i++) {
        if (manifest[i] !== 0x30 || manifest[i + 1] !== 0)
            continue;
        if (manifest[i + 2] !== 0x2e || manifest[i + 3] !== 0)
            continue;
        let s = "";
        let j = i;
...
//# sourceMappingURL=apk-meta.js.map