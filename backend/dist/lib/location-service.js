...
exports.composeDisplayAddress = composeDisplayAddress;
exports.photonFeatureToSuggestion = photonFeatureToSuggestion;
...
function composeDisplayAddress(parts) {
...
function photonFeatureToSuggestion(feature) {
...
function nominatimHitToSuggestion(hit) {
    ...
    const city = String(a.city || a.town || a.village || a.municipality || "").trim() || undefined;
    ...
    const displayAddress = composeDisplayAddress({ houseNumber, street, postcode, city, country }) ||
...
function filterAndDedupeSuggestions(items, countryCode) {
...
async function photonAutocomplete(opts) {
    ...
    const data = (await fetchJson(`${photonBaseUrl()}/api/?${params.toString()}`));
...
async function autocompleteAddress(opts) {
