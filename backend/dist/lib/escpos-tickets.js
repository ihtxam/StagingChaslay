...
exports.reservationTicketEscPos = reservationTicketEscPos;
exports.kitchenTicketEscPos = kitchenTicketEscPos;
exports.orderNotificationTicketEscPos = orderNotificationTicketEscPos;
exports.deliverySlipEscPos = deliverySlipEscPos;
...
function reservationTicketEscPos(opts) {
    ...
    if (opts.tableLabel)
        lines.push(padLine("Table", String(opts.tableLabel), width));
    if (opts.status)
        lines.push(padLine("Status", String(opts.status), width));
    if (opts.notes?.trim()) {
        lines.push("---");
...
function kitchenTicketEscPos(opts) {
    const width = opts.paperWidthMm === 58 ? 32 : 42;
    const lines = [
    ...
    if (opts.customerName)
        lines.push(padLine("Guest", opts.customerName, width));
    if (opts.customerPhone)
        lines.push(padLine("Tel", opts.customerPhone, width));
    if (opts.channel)
        lines.push(padLine("Channel", opts.channel, width));
    if (opts.scheduledFor)
        lines.push(padLine("When", formatWhen(opts.scheduledFor), width));
...
function orderNotificationTicketEscPos(opts) {
    const width = opts.paperWidthMm === 58 ? 32 : 42;
    const lines = [
        opts.businessName || "",
        String(opts.orderSource || "ONLINE").toUpperCase(),
    ...
    if (opts.items.length > 16)
        lines.push(`+${opts.items.length - 16} more`);
    if (opts.total != null) {
...
function deliverySlipEscPos(opts) {
    const width = opts.paperWidthMm === 58 ? 32 : 42;
    const lines = [
        opts.businessName || "",
        String(opts.orderNumber || "-"),
...
//# sourceMappingURL=escpos-tickets.js.map