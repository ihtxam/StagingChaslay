...
exports.isJetConnectOrderPayload = isJetConnectOrderPayload;
exports.mapJetConnectWebhookBody = mapJetConnectWebhookBody;
exports.mapJustEatWebhookBody = mapJustEatWebhookBody;
exports.mapUberEatsWebhookBody = mapUberEatsWebhookBody;
exports.isUberNotificationOnly = isUberNotificationOnly;
function asObj(v) {
...
function mapLineItems(raw) {
    ...
    return rows
        ...
        const unitPrice = pickNum(r.unitPrice, r.price, r.unit_price, r.totalPrice, r.total_price, nested.price, nested.unitPrice);
...
function flattenJetConnectItems(raw) {
    const lines = [];
    const walk = (items) => {
...
function isJetConnectOrderPayload(body) {
 ...
 */
function mapJetConnectWebhookBody(body) {
    const root = asObj(body);
    ...
    return {
        externalOrderId,
        fulfillmentChannel: method === "collection" || method.includes("pickup") ? "takeaway" : "delivery",
        ...
        scheduledFor: pickStr(fulfilment.CustomerDueDate, fulfilment.customerDueDate, fulfilment.PrepareFor, fulfilment.prepareFor, root.PlacedDate, root.placedDate) || null,
 ...
 */
function mapJustEatWebhookBody(body) {
    ...
    const externalOrderId = pickStr(data.externalOrderId, data.orderId, data.order_id, data.id, data.Id, root.orderId, root.id);
    if (!externalOrderId)
        return body;
    const items = mapLineItems(data.items ||
        data.lineItems ||
        data.products ||
        data.Products ||
        data.orderItems ||
        data.OrderItems);
    const fulfillmentRaw = pickStr(data.fulfillmentChannel, data.fulfillment, data.orderType, data.serviceType, delivery.type).toLowerCase();
    return {
        externalOrderId,
        fulfillmentChannel: fulfillmentRaw.includes("collection") || fulfillmentRaw.includes("pickup")
            ? "takeaway"
        ...
        shippingAddress: formatAddress(delivery.address || delivery) ||
 ...
 */
function mapUberEatsWebhookBody(body) {
    const root = asObj(body);
    const meta = asObj(root.meta || root.webhook_meta);
    const order = asObj(root.order || root.resource || root.data);
    const externalOrderId = pickStr(order.id, order.order_id, meta.resource_id, root.resource_id, root.externalOrderId, root.id);
    if (!externalOrderId)
        return body;
    const cart = asObj(order.cart || order.payment);
    const items = mapLineItems(order.items ||
        order.line_items ||
        cart.items ||
        cart.line_items ||
        order.order_items);
    ...
    return {
        ...
        shippingAddress: formatAddress(delivery.location || delivery.address) ||
...
function isUberNotificationOnly(mapped) {
...
//# sourceMappingURL=delivery-platform-webhook-mappers.js.map