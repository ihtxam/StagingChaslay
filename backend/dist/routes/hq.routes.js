...
router.get("/hq/catalog/versions", async (req, res) => {
    try {
        const versions = await hq_catalog_service_1.HqCatalogService.listVersions(req.merchantId);
...
router.post("/hq/catalog/versions", async (req, res) => {
    try {
        const body = req.body || {};
        const version = await hq_catalog_service_1.HqCatalogService.createVersion(req.merchantId, {
            name: body.name,
...
router.post("/hq/catalog/push", async (req, res) => {
    try {
        const body = req.body || {};
        const result = await hq_catalog_service_1.HqCatalogService.pushToLocations(req.merchantId, {
...
router.get("/hq/catalog/links/:locationId", async (req, res) => {
    try {
        const links = await hq_catalog_service_1.HqCatalogService.listLocationLinks(req.merchantId, req.params.locationId);
...
router.post("/hq/bulk-pricing/preview", async (req, res) => {
    try {
        const body = req.body || {};
        const preview = await hq_catalog_service_1.BulkPricingService.preview(req.merchantId, {
...
router.post("/hq/bulk-pricing/apply", async (req, res) => {
    try {
        const body = req.body || {};
        const result = await hq_catalog_service_1.BulkPricingService.apply(req.merchantId, String(body.previewToken || ""), {
            staffId: req.user?.staffId || null,
...
router.get("/hq/analytics", async (req, res) => {
    try {
        ...
        const analytics = await pos_reports_service_1.PosReportsService.getOrgAnalytics(req.merchantId, {
            preset: preset,
        });
...
router.put("/hq/menus/:menuId", async (req, res) => {
    try {
...
router.get("/hq/bulk-pricing/jobs", async (req, res) => {
    try {
        const jobs = await hq_catalog_service_1.BulkPricingService.listJobs(req.merchantId);
...
//# sourceMappingURL=hq.routes.js.map