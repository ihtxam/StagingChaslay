...
router.post("/login", async (req, res) => {
    try {
        ...
        const result = await auth_service_1.AuthService.loginAny(String(email), String(password));
...
router.post("/merchant/register", async (req, res) => {
    try {
        ...
        const merchant = await auth_service_1.AuthService.registerMerchant(email, password, name, businessName, category);
        res.status(201).json({
            success: true,
...
router.post("/merchant/login", async (req, res) => {
    try {
        ...
        const result = await auth_service_1.AuthService.loginMerchant(email, password);
        res.json({
            success: true,
            token: result.token,
            merchant: result.merchant,
 ...
 */
router.post("/superadmin/register", auth_middleware_1.verifyToken, auth_middleware_1.requireSuperadmin, async (req, res) => {
    try {
...
router.post("/forgot-password", async (req, res) => {
    try {
        ...
        const result = await password_reset_service_1.PasswordResetService.requestReset(email, clientIp(req));
        res.json(result);
    }
    catch (error) {
        if (error instanceof password_reset_service_1.PasswordResetRateLimitError) {
            return res.status(429).json({ error: error.message });
        }
        console.error("Error requesting password reset:", error);
        res.json({ success: true, message: password_reset_service_1.PasswordResetService.genericSentMessage() });
    }
});
/**
...
router.get("/reset-password/:token", async (req, res) => {
    try {
        const preview = await password_reset_service_1.PasswordResetService.previewToken(req.params.token);
...
router.post("/reset-password", async (req, res) => {
    try {
        ...
        const result = await password_reset_service_1.PasswordResetService.applyReset(String(token || ""), nextPassword);
 ...
 */
router.post("/change-own-password", auth_middleware_1.verifyToken, async (req, res) => {
    try {
        if (!req.user) {
            return res.status(401).json({ error: "Not authenticated" });
        }
        const { currentPassword, newPassword } = req.body || {};
        await password_reset_service_1.PasswordResetService.changeOwnPassword(req.user, String(currentPassword || ""), String(newPassword || ""));
 ...
 */
router.post("/reset-login-password", async (req, res) => {
    try {
        const { email, newPassword, role } = req.body || {};
        const allowed = ["merchant", "staff", "reseller", "superadmin"];
        if (!allowed.includes(role)) {
            return res.status(400).json({ error: "role must be merchant, staff, reseller, or superadmin" });
        }
        const result = await auth_service_1.AuthService.resetLoginPasswordByEmail(role, String(email || ""), String(newPassword || ""));
...
router.get("/me", auth_middleware_1.verifyToken, async (req, res) => {
    try {
        ...
        if (req.user.role === "merchant") {
            ...
            const merchant = await auth_service_1.AuthService.getMerchantById(merchantId);
            res.json({
                user: {
                    id: merchant.id,
        ...
        if (req.user.role === "staff" && req.user.staffId && req.user.merchantId) {
            ...
            const { readInventoryAddonEnabled } = await Promise.resolve().then(() => __importStar(require("@/lib/inventory-addon")));
            const { readSignageAddon } = await Promise.resolve().then(() => __importStar(require("@/lib/signage-addon")));
 ...
 */
router.post("/change-password", auth_middleware_1.verifyToken, auth_middleware_1.requireMerchant, async (req, res) => {
    try {
        ...
        await auth_service_1.AuthService.updateMerchantPassword(req.user.id, newPassword);
        res.json({
            success: true,
...
router.get("/invite/:token", async (req, res) => {
    try {
        const { MerchantInviteService } = await Promise.resolve().then(() => __importStar(require("@/services/merchant-invite.service")));
