...
class AnalyticsService {
    ...
    static async getPlatformOverview() {
        ...
        try {
            ...
            const activeLicenses = licenses.filter((l) => l.status === "active" && l.expiresAt > now).length;
            ...
            const merchantsThisMonth = merchants.filter((m) => m.createdAt.toISOString().substring(0, 7) === thisMonthKey).length;
            const merchantsPrevMonth = merchants.filter((m) => m.createdAt.toISOString().substring(0, 7) === prevMonthKey).length;
            const platformGrowth = merchantsPrevMonth > 0
    ...
    static async getRevenueAnalytics(startDate, endDate) {
        ...
        try {
            ...
            if (startDate && endDate) {
                orders = orders.filter((o) => o.createdAt >= startDate && o.createdAt <= endDate);
            }
            const totalRevenue = orders.reduce((sum, order) => sum + parseFloat(order.total.toString()), 0);
     ...
     */
    static async getMerchantGrowthAnalytics(months = 12) {
        const db = (0, db_1.getDb)();
        try {
            const merchants = await db.query.merchants.findMany();
            const monthlyGrowth = {};
     ...
     */
    static async getLicenseRenewalForecast(daysAhead = 90) {
        const db = (0, db_1.getDb)();
        try {
            const now = new Date();
