...
exports.CmsService = exports.normalizePuckData = exports.CMS_TEMPLATES = void 0;
exports.normalizeCmsBlocks = normalizeCmsBlocks;
...
function selfContainedHtml(title, opts) {
    const year = new Date().getFullYear();
    const safe = escapeHtml(title);
    const features = (opts.features || [])
        .map((f) => `<article style="background:#26201c;border:1px solid #352e28;border-radius:14px;padding:1.1rem 1.2rem"><h3 style="margin:0 0 .35rem;font-size:1rem">${escapeHtml(f.title)}</h3><p style="margin:0;color:#a89a88;font-size:.9rem">${escapeHtml(f.description)}</p></article>`)
        .join("");
...
function openPageScaffold(title, blocks, html, theme) {
    const config = {
        name: title,
        blocks,
...
function restaurantData(shopName) {
...
function foodTruckData(shopName) {
    return openPageScaffold(shopName, [
        {
            id: bid(),
            type: "navbar",
...
function cafeLightHtml(title, opts) {
    const year = new Date().getFullYear();
    const safe = escapeHtml(title);
    return `<!DOCTYPE html><html lang="en"><head><meta charset="utf-8"/><meta name="viewport" content="width=device-width,initial-scale=1"/><title>${safe}</title>
...
function cafeClassicData(shopName) {
    return openPageScaffold(shopName, [
        {
            id: bid(),
            type: "navbar",
...
function bistroLightData(shopName) {
    const data = cafeClassicData(shopName);
    const blocks = data.config.blocks.map((b) => b.type === "hero"
        ? {
            ...b,
            props: {
                ...b.props,
...
function normalizeCmsBlocks(raw, fallbackTitle = "") {
    if (raw && typeof raw === "object" && !Array.isArray(raw)) {
        const o = raw;
        if (o.engine === "openpage" &&
            o.config &&
            typeof o.config === "object" &&
            Array.isArray(o.config.blocks) &&
            ...
            if (o.locales && typeof o.locales === "object") {
                next.locales = o.locales;
            }
            return next;
        }
        // Legacy Puck `{ content, root }` → OpenPage starter (rebuild in editor)
        if (Array.isArray(o.content)) {
            const title = String(o.root?.props?.title || fallbackTitle) ||
                "Homepage";
...
/** @deprecated alias */
exports.normalizePuckData = normalizeCmsBlocks;
class CmsService {
    static listTemplates() {
        return exports.CMS_TEMPLATES.map(({ key, name, description }) => ({ key, name, description }));
    }
    static async listPages(merchantId) {
        const db = (0, db_1.getDb)();
        return db.query.cmsPages.findMany({
            where: (0, drizzle_orm_1.eq)(db_1.schema.cmsPages.merchantId, merchantId),
            orderBy: [(0, drizzle_orm_1.desc)(db_1.schema.cmsPages.isHomepage), (0, drizzle_orm_1.asc)(db_1.schema.cmsPages.title)],
        });
    }
    static async getPage(merchantId, pageId) {
        const db = (0, db_1.getDb)();
        const page = await db.query.cmsPages.findFirst({
            where: (0, drizzle_orm_1.and)((0, drizzle_orm_1.eq)(db_1.schema.cmsPages.id, pageId), (0, drizzle_orm_1.eq)(db_1.schema.cmsPages.merchantId, merchantId)),
        });
        if (!page)
    ...
    static async createPage(merchantId, input) {
        const db = (0, db_1.getDb)();
        ...
        const template = exports.CMS_TEMPLATES.find((t) => t.key === (input.templateKey || "food_truck"));
        ...
        const existing = await db.query.cmsPages.findFirst({
            where: (0, drizzle_orm_1.and)((0, drizzle_orm_1.eq)(db_1.schema.cmsPages.merchantId, merchantId), (0, drizzle_orm_1.eq)(db_1.schema.cmsPages.slug, slug)),
        });
        if (existing)
        ...
        if (isHomepage) {
            await db
                .update(db_1.schema.cmsPages)
                .set({ isHomepage: false, updatedAt: new Date() })
                .where((0, drizzle_orm_1.and)((0, drizzle_orm_1.eq)(db_1.schema.cmsPages.merchantId, merchantId), (0, drizzle_orm_1.eq)(db_1.schema.cmsPages.isHomepage, true)));
        }
        const status = input.status === "published" ? "published" : "draft";
        const blocks = input.blocks !== undefined
            ? normalizeCmsBlocks(input.blocks, title)
            : template
                ? template.data(merchant.name)
                : emptyData(title);
        const [page] = await db
            .insert(db_1.schema.cmsPages)
            .values({
            merchantId,
            title,
            slug,
    ...
    static async updatePage(merchantId, pageId, input) {
        const db = (0, db_1.getDb)();
        ...
        if (input.seoTitle !== undefined)
            patch.seoTitle = input.seoTitle ? input.seoTitle.slice(0, 200) : null;
    ...
    static async deletePage(merchantId, pageId) {
        const db = (0, db_1.getDb)();
        const page = await this.getPage(merchantId, pageId);
        await db
            .delete(db_1.schema.cmsPages)
            .where((0, drizzle_orm_1.and)((0, drizzle_orm_1.eq)(db_1.schema.cmsPages.id, pageId), (0, drizzle_orm_1.eq)(db_1.schema.cmsPages.merchantId, merchantId)));
        if (page.isHomepage) {
            await db
                .update(db_1.schema.merchants)
    ...
    static async getPublishedHomepage(merchantId) {
        const db = (0, db_1.getDb)();
        const page = await db.query.cmsPages.findFirst({
    ...
    static async getPublishedBySlug(merchantId, slug) {
        const db = (0, db_1.getDb)();
        const page = await db.query.cmsPages.findFirst({
            where: (0, drizzle_orm_1.and)((0, drizzle_orm_1.eq)(db_1.schema.cmsPages.merchantId, merchantId), (0, drizzle_orm_1.eq)(db_1.schema.cmsPages.slug, slugify(slug)), (0, drizzle_orm_1.eq)(db_1.schema.cmsPages.status, "published")),
        });
        if (!page)
            return null;
        return { ...page, blocks: normalizeCmsBlocks(page.blocks, page.title) };
    }
    static async updateSiteSettings(merchantId, input) {
        const db = (0, db_1.getDb)();
        const patch = { updatedAt: new Date() };
        if (input.customDomain !== undefined) {
            patch.customDomain = (0, domain_1.normalizeCustomDomain)(input.customDomain);
        }
    ...
    static async getSiteSettings(merchantId) {
        const db = (0, db_1.getDb)();
        const merchant = await db.query.merchants.findFirst({
            where: (0, drizzle_orm_1.eq)(db_1.schema.merchants.id, merchantId),
        });
