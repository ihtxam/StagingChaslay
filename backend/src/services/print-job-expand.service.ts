...
function looksLikeLabelPrinterName(name?: string | null): boolean {
  const n = String(name || "").toLowerCase();
  if (!n.trim()) return false;
  if (/niimbot|\bk3\b|\bb21\b|\bd11\b|\bb1\b|\bd110\b|\bb3s\b/.test(n)) return true;
...
function printersForRole(
  ...
  const list = (printers || []).filter(
    (p) => p.enabled !== false && p.name && !looksLikeLabelPrinterName(p.name)
  );
