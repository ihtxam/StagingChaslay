...
import { renderInvoicePdf, type PdfInput } from "../services/invoice.service";

function pdfPageContent(buf: Buffer): string {
  ...
  for (const m of raw.matchAll(/\/Length (\d+)\s*\/Filter \/FlateDecode\s*>>\s*stream\r?\n/g)) {
    ...
    try {
      parts.push(zlib.inflateSync(buf.subarray(dataStart, dataStart + Number(m[1]))).toString("latin1"));
    } catch {
...
function sampleInput(partial: Partial<PdfInput> = {}): PdfInput {
  return {
    labels,
    lang: "en",
    merchant: {
...
async function main() {
  ...
  const matrix = swissQrModuleRuns(payload);
  ...
  const withQr = await renderInvoicePdf(sampleInput({ qrPayload: payload }));
  ...
  console.log(
    "invoice with QR: bytes",
    withQr.length,
    "pages",
    (withQr.toString("latin1").match(/\/Type \/Page\b/g) || []).length,
    "rects",
    rects
  );

  const missing = await renderInvoicePdf(
    sampleInput({
      qrPayload: "",
  ...
  assert(
    missingText.includes("Swiss QR-bill unavailable") || missingText.includes("Add an IBAN"),
    `fallback message missing when IBAN is empty (${missingText.slice(0, 180)})`
  );
